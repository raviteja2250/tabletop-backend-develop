...
class FileTypeValidator:
    ...
    def __call__(self, fileobj):
        detected_type = magic.from_buffer(
            fileobj.read(self._READ_SIZE), mime=True)
        ...
        if detected_type in ('application/octet-stream', 'application/vnd.ms-office'):
            detected_type = self._check_word_or_excel(
                fileobj, detected_type, extension)
    ...
    def _normalize(self, allowed_types):
        ...
        for allowed_type in allowed_types:
            allowed_type = allowed_type.decode() if isinstance(
                allowed_type, bytes) else allowed_type
    ...
    def _check_word_or_excel(self, fileobj, detected_type, extension):
        ...
        """
        word_strings = ['Microsoft Word',
                        'Microsoft Office Word', 'Microsoft Macintosh Word']
        excel_strings = ['Microsoft Excel',
                         'Microsoft Office Excel', 'Microsoft Macintosh Excel']
        office_strings = ['Microsoft OOXML']

        file_type_details = magic.from_buffer(fileobj.read(self._READ_SIZE))
